...
Domains that are available are: `core`, `work`, `work-items`, `search`, `test-plans`, `repositories`, `wiki`, `pipelines`, `advanced-security`, `dashboards`, `policy`, `task-agent`, `release`, `wit-process`, `notification`, `security-roles`, `project-analysis`
