...
**Removed in the fork**, so upstream changes to these files need no port.

- `.github/workflows/version-update.yml` — never did anything: the label step read `GITHUB_EVENT_PULL_REQUEST_LABELS`, which Actions does not set, and kept `VERSION_TYPE` in its own process instead of `$GITHUB_ENV`, so the bump and push steps were always skipped. Working, it would have pushed from a pull request straight to `main`. The fork bumps `package.json` in the pull request itself, and CI checks that `src/version.ts` matches.
